...
export function makeEnemy(type, x, z) {
  ...
  return {
    ...
    navX: 0,            // 길찾기가 마지막으로 알려 준 방향
    navZ: 0,
    navT: 0,            // 그 방향을 다시 물어볼 때까지 남은 시간
...
/* 길찾기에 길을 다시 물어보는 간격.
   ...
   매 프레임 물어보면 안 된다. 격자 위의 두 길이 값이 비슷할 때 —
   벽 하나를 사이에 두고 왼쪽으로 도는 길과 오른쪽으로 도는 길의 비용이
   엇비슷할 때 — 적이 몇 센티만 움직여도 답이 반대로 바뀐다. 그러면
   한 프레임은 서쪽, 다음 프레임은 동쪽으로 밀려 제자리에서 떤다.
   실제로 탑·보루·참호에서 그렇게 갇힌 적을 봤다.

   사람도 갈림길에서 한 발짝마다 마음을 바꾸지는 않는다. 한번 정하면
   잠깐은 밀고 간다. 0.2초면 크롤러가 한 걸음 가까이 움직여서, 다시
   물어볼 때쯤엔 갈림길에서 확실히 벗어나 있다.

   가까이 붙으면 다시 매 프레임 묻는다. 코앞에서는 길찾기가 어차피
   플레이어를 곧장 가리키고, 여기서 굳은 방향을 붙들면 사람을 지나쳐
   맴돈다. */
const NAV_HOLD = 0.2
const NAV_CLOSE = 3.0
...
export function tickEnemy(enemy, ctx, dt) {
  ...
  if (moveScale !== 0 && dist > 1e-4) {
    ...
    /* 어느 쪽으로 갈까 — 원칙은 플레이어를 향해 곧장이다.

       곧장 가면 안 되는 자리에서만 길찾기가 끼어든다(navgrid.js). 그런
       자리는 벽 따라가기로는 못 푼다 — 막다른 주머니에서는 나갈 길이
       뒤에 있고, 빙 돌아야 하는 배치에서는 돌아야 할 방향이 당장은
       플레이어에서 멀어지는 쪽이라 몇 걸음 가다 되돌아온다.

       트인 데서는 여전히 곧장 간다. 길찾기가 늘 끼어들면 움직임이
       격자 모양으로 각지고, 그에 맞춰 재 둔 난이도도 흔들린다. */
    let ax = dx / dist
    let az = dz / dist
    if (sign > 0 && ctx.navDir) {
      e.navT -= dt
      if (e.navT <= 0 || dist < NAV_CLOSE) {
        const nd = ctx.navDir(e.x, e.z, t.radius, e.y || 0)
        e.navX = nd ? nd.x : 0
        e.navZ = nd ? nd.z : 0
        /* 마리마다 조금씩 어긋나게 해서 같은 프레임에 몰려 묻지 않게 한다 */
        e.navT = NAV_HOLD + (hash32(e.id) % 7) * 0.01
      }
      if (e.navX !== 0 || e.navZ !== 0) { ax = e.navX; az = e.navZ }
    }
    ...
    /* 벽에 막혔을 때 — 한쪽 방향을 정해 "한동안 그쪽으로만" 돈다.
       ...
       이건 이제 큰 길이 아니라 잔손질이다. 어느 쪽으로 돌아가야 하는지는
       길찾기가 정하고(navgrid.js), 여기서 하는 일은 그 방향으로 가다가
       몸이 벽 모서리에 걸렸을 때 흘려보내는 것이다.

       한때는 이것뿐이었다. 주석에도 "기둥 몇 개짜리 아레나에서는 이걸로
       충분하고, 모든 스폰에서 도달하는지 테스트가 지키니 그게 깨지면
       그때 길찾기를 들이면 된다"고 적어 두었다. 맵을 마흔 장으로 늘리자
       정확히 그 테스트가 깨졌고, 그래서 길찾기를 들였다. */
    if (!blocked) {
