/* ==================================================================
   ...
   통나무는 10분이면 썩는다(§7.1). 목적이 분명하다 — **모아 두기를
   막는다**. 저장고 없이 통나무를 쌓아 두는 빌드를 원천봉쇄해서,
   고철 2개짜리 저장고가 초반의 숨은 관문이 되게 한다.

   ── 왜 "비율로 조금씩"이 아니라 "묶음"인가

   "10분에 전부"를 개수로 나눠 떼면 남은 개수가 줄수록 속도도 줄어서,
   20개가 0이 되는 데 36분이 걸린다. 조화급수라 영영 안 끝난다.

   그래서 들어온 시각별로 묶어서 센다. 10분 전에 들어온 묶음이 통째로
   사라진다. 통나무 하나하나에 시각을 달지 않아도 되고, "10분 뒤에
   사라진다"가 말 그대로 성립한다.

   묶음은 addItem/removeItem 에 갈고리를 걸지 않고 매 프레임 대조해서
   맞춘다. 덕분에 인벤토리 코드는 부패를 몰라도 된다.
   ================================================================== */
import { BALANCE } from '../data/index.ts'
import type { GameState } from '../core/GameState.ts'
import { countItem, removeItem, toast } from '../core/GameState.ts'
import { farmGrowDays } from '../entities/Crew.ts'
...
/** 검사용 — 지금 추적 중인 통나무 총량 */
export function trackedLogs(s: GameState): number {
...
export function harvestFarms(s: GameState): void {
  const need = farmGrowDays(s)
  for (const b of s.buildings) {
    if (b.type !== 'farm') continue
    if ((b.timerDays ?? 0) < need) continue
    b.timerDays = 0
    /* 안 거둔 작물은 다음 주기에 썩는다 */
    if (b.crop) {
      b.crop = null
