/* ==================================================================
   ...
   할 일은 둘이다 — 배치를 부르고, 부딪히는 것 목록을 만든다.

   막는 것을 미리 뽑아 두는 이유는 성능이다. 큰 나무만 100그루가 넘는데
   매 프레임 전체를 훑으면 사람과 짐승과 약탈자와 거미가 다 같이 훑는다.
...
/** 불시착한 본선. 중앙 고정이고 부딪힌다. */
export const SHIP: Solid = { x: CENTER, z: CENTER, r: 2.4 }

/** 큰 나무만 막는다. 작은 나무까지 막으면 평원이 걸어 다니기 답답하다. */
...
export function buildSolids(s: GameState): Solid[] {
  ...
  /* 벽도 막는다. 습격 방어의 핵심이라 실제로 통행을 막아야 한다. */
  for (const b of s.buildings) {
    if (b.type === 'wall_wood' || b.type === 'wall_steel') {
      out.push({ x: b.x, z: b.z, r: 0.55 })
    }
  }
  return out
}
