/* ==================================================================
   ...
   기획서는 Howler 를 적었지만 Web Audio 를 직접 쓴다. 필요한 것이
   공간 오디오와 로우패스 필터인데 그건 Web Audio 의 기본 기능이고,
   음원 파일이 하나도 없어서(전부 합성) 로더가 할 일이 없다.

   §20.5 의 규칙이 더 중요하다 — **밤에는 BGM 이 없다.** 발소리와
   거미 소리만 남긴다. 침묵이 가장 무섭다.

   습격 밤에는 북 리듬만 깐다. 멀리서 시작해 점점 가까워진다(§20.4).
   ================================================================== */
...
export class Audio {
  ...
  play(event: string): void {
    switch (event) {
      ...
      case 'crossbow': this.tone(900, 0.07, 'sawtooth', 0.14, 400); break
      case 'reload': this.tone(300, 0.06, 'square', 0.14); break
      case 'melee': this.noise(0.09, 0.24, 1200); break
      case 'beastHit': case 'raiderHit': this.tone(180, 0.16, 'sawtooth', 0.24, 90); break
      case 'beastDown': case 'raiderDown': this.tone(240, 0.34, 'sawtooth', 0.22, 70); break
      /* 우주복 파손 — 유리 균열(§20.4) */
      ...
      case 'crewStoke': this.noise(0.2, 0.16, 380); break
      case 'crewShot': this.noise(0.08, 0.3, 1400); break
      case 'fireLevelUp': this.tone(330, 0.45, 'sine', 0.22, 700); break
      case 'fireDrop': this.tone(300, 0.3, 'sine', 0.16, 150); break
      /* 불 꺼짐 — 앰비언스 정지 → 정적 → 거미 울음(§20.4) */
      case 'fireOut':
        this.tone(200, 0.25, 'sine', 0.2, 60)
        setTimeout(() => this.tone(70, 1.4, 'sawtooth', 0.26, 40), 1000)
        break
      case 'fireLit': this.noise(0.35, 0.24, 500); break
      ...
      case 'cacheOpen': this.tone(180, 0.5, 'square', 0.2, 300); break
      case 'labEnter': this.tone(90, 0.7, 'square', 0.26, 60); break
      case 'crewRescued': this.tone(440, 0.6, 'triangle', 0.26, 880); break
      case 'nightWarning':
      ...
      case 'raidWarn': this.tone(90, 1.2, 'sine', 0.24, 60); break
      case 'raidStart': this.noise(0.6, 0.4, 160); break
      case 'raidFailed': this.tone(120, 1.2, 'sawtooth', 0.28, 50); break
      case 'raidCleared': this.tone(360, 0.6, 'triangle', 0.22, 720); break
      case 'wallBroke': this.noise(0.4, 0.4, 300); break
      ...
      case 'salvageHit': this.tone(660, 0.09, 'sine', 0.18, 900); break
      case 'salvageMiss': this.tone(160, 0.18, 'square', 0.16, 80); break
      case 'salvageLevel': this.tone(520, 0.4, 'triangle', 0.22, 900); break
      case 'weather': this.noise(0.8, 0.18, 420); break
      case 'stormWarn': this.tone(1200, 0.3, 'sine', 0.14, 1600); break
      case 'lightning': this.noise(0.5, 0.6, 900); this.tone(60, 0.5, 'square', 0.32, 30); break
      case 'barrier': this.tone(880, 0.12, 'sine', 0.1, 660); break
  ...
  /** 거미 마찰음. 가까울수록 커진다 — 보이기 전부터 들린다(§20.4). */
  ...
  /** 습격 밤의 북. 거리가 가까울수록 빨라진다(§20.5). */
  drums(active: boolean, intensity: number, dt: number): void {
    if (!this.enabled) return
    this.drumOn = active
    if (!active) return
    const period = 0.85 - 0.35 * Math.min(1, intensity)
    this.drumT -= dt
    if (this.drumT <= 0) {
      this.drumT = period
      this.tone(58, 0.22, 'sine', 0.16 + 0.14 * intensity, 38)
      this.noise(0.1, 0.08 + 0.06 * intensity, 180)
    }
  }

  get drumming(): boolean {
    return this.drumOn
  }
}
