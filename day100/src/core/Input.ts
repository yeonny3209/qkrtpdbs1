/* ==================================================================
   ...
   코드는 물리 키(KeyW)로 본다. 한글 자판이나 AZERTY 에서도 손 모양이
   같은 자리에 오게 하려는 것이다.
   ================================================================== */
...
export class Input {
  ...
  private onKeyDown = (e: KeyboardEvent) => {
    if (e.repeat) return
    /* 게임 중에는 브라우저 기본 동작(스페이스 스크롤 등)을 막는다.
       단 F5·F12·개발자 도구 조합은 살려 둔다. */
  ...
  private onBlur = () => {
    /* 탭을 벗어나면 키가 눌린 채로 남는다. 돌아왔을 때 혼자 달리고
       있으면 스태미나가 이미 비어 있어서, 밤이면 그대로 죽는다. */
    this.held.clear()
