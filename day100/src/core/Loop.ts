/* ==================================================================
   ...
   로직은 고정 60fps, 그리기는 가변이다. 이렇게 나누는 이유는
   두 가지다.

     - 120Hz 모니터에서 산소가 두 배로 빨리 닳지 않는다
     - 시드가 같으면 같은 결과가 나온다. dt 가 프레임마다 다르면
       난수를 안 써도 결과가 갈린다

   프레임 시간에 상한(0.25초)을 두는 것도 중요하다. 탭을 다른 데
   두었다 돌아오면 dt 가 몇 초씩 들어오는데, 그대로 밀어 넣으면
   한 프레임에 밤이 통째로 지나가고 거미가 순간이동한다.
   ================================================================== */
