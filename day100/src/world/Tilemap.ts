/* ==================================================================
   ...
   좌표는 전부 "타일" 단위다. 픽셀은 그리는 순간에만 곱한다.
   속도 4.0 이 초당 4타일이라는 뜻이 되어, 기획서의 숫자를 코드에서
   그대로 읽을 수 있다.

   지형은 저장하지 않는다. 존은 중심에서의 거리로 정해지므로 함수
   하나면 되고, 192×192 배열을 들고 다닐 이유가 없다. 대신 표면
   무늬만 시드 해시로 만들어 같은 시드면 같은 그림이 나오게 한다.
   ================================================================== */
import { BALANCE, zoneAt, type ZoneDef } from '../data/index.ts'
...
export function tileIndex(tx: number, tz: number): number {
  return tz * TILES + tx
}
...
export function zoneOf(x: number, z: number): ZoneDef {
  return zoneAt(distanceFromCenter(x, z))
}

/* 타일마다 살짝 다른 얼룩. 시드 해시라 저장할 것이 없고, 카메라가
   움직여도 무늬가 미끄러지지 않는다. */
export function tileTint(seed: string, tx: number, tz: number): number {
  const h = hash32(`${seed}:${tx}:${tz}`)
  return (h % 1000) / 1000
}

/** 맵 밖으로 못 나가게 자른다. 가장자리 2타일은 여유를 둔다. */
...
/* 원끼리 밀어내기. 지형이 축에 나란한 상자가 아니라 나무·바위 같은
   둥근 것들이라, 원-원 하나로 충분하다. 여러 개에 동시에 끼는
   경우를 위해 몇 번 되풀이한다. */
export function pushOut(
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    for (const s of solids) {
      ...
      if (d < 1e-6) {
        /* 정확히 겹쳤다. 아무 방향으로나 한 번 밀어야 나눗셈이 터지지 않는다. */
        px += need
        moved = true
        continue
      }
...
/** 선분이 막힌 것에 걸리는가 — 거미의 "장애물만 회피"에 쓴다. */
