/* ==================================================================
   ...
   맵 가장자리에서는 카메라를 가둔다. 안 그러면 화면 절반이 허공이라
   플레이어가 "여기가 끝인가?"를 화면으로 알 수 없다.
   ================================================================== */
...
export class Camera {
  ...
  toScreenX(x: number): number {
    return (x - this.x) * TILE + this.w / 2
  }

  toScreenY(z: number): number {
    return (z - this.z) * TILE + this.h / 2
  }

  /** 화면(픽셀) → 월드(타일). 마우스 조준에 쓴다. */
  toWorldX(px: number): number {
    return (px - this.w / 2) / TILE + this.x
  }

  toWorldZ(py: number): number {
    return (py - this.h / 2) / TILE + this.z
  }
