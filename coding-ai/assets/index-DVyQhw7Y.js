...
`+t:``}`}function g(e,t){return e?`/* 브라우저에 남긴다 — 서버가 없어도 되고, 이 기기에만 저장된다.
...
}`:`/* 저장을 쓰지 않는 설정으로 만들어졌습니다.
   나중에 남기고 싶어지면 아래 두 함수의 속을 채우면 됩니다. */
function load(fallback) { return fallback; }
function save() {}`}var _=`/* 짧은 알림음. 소리 파일 없이 브라우저가 직접 음을 만든다. */
function beep() {
  try {
...
}`;function v({title:e,themeId:t,body:n,script:r,extraCss:i=``}){return`<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8">
...
`}function y(e){let t=Array.isArray(e.todoFields)?e.todoFields:[],n=t.includes(`none`),r=!n&&t.includes(`due`),i=!n&&t.includes(`priority`),a=!n&&t.includes(`tag`),o=e.title||`할 일 목록`,s=e.save===`yes`,c=[`      <input id="text" placeholder="할 일을 적어보세요" style="flex:1;min-width:160px">`,r?`      <input id="due" type="date" aria-label="마감일">`:``,i?`      <select id="pri" aria-label="중요도">
        <option value="1">보통</option>
        <option value="2">🔥 높음</option>
        <option value="0">낮음</option>
      </select>`:``,a?`      <input id="tag" placeholder="분류" style="width:110px" aria-label="분류">`:``,`      <button class="primary" id="add">추가</button>`].filter(Boolean).join(`
`),l=`  <h1>${p(o)}</h1>
...
`:``}
  ...
  <p class="sub" id="count" style="margin-top:16px"></p>`,u=i?`var PRI = { 2: { text: '높음', color: '#ef4444' }, 1: { text: '보통', color: 'var(--muted)' }, 0: { text: '낮음', color: 'var(--muted)' } };
`:``,d=`${g(s,`todo.`+(e.title||`default`))}
...
${r?`/* 오늘 날짜를 'YYYY-MM-DD' 로 — 날짜 입력칸이 쓰는 형식과 맞춘다.
...
${r?`  item.due = document.getElementById('due').value || '';
`:``}${i?`  item.pri = Number(document.getElementById('pri').value);
`:``}${a?`  item.tag = document.getElementById('tag').value.trim();
`:``}  items.push(item);
  document.getElementById('text').value = '';
${a?`  document.getElementById('tag').value = '';
`:``}  save(items);
  render();
...
`:`  var shown = items;
`}${i?`  /* 중요한 것이 위로. 같은 중요도면 넣은 순서를 지킨다 —
...
`:``}${a?`      if (it.tag) meta.push('<span style="color:var(--muted)">🏷 ' + esc(it.tag) + '</span>');
...
${a?`  else if (t.dataset.filter) { filter = t.dataset.filter; render(); }
`:``}});
...
render();`,f=[r&&`마감일(지나면 빨갛게)`,i&&`중요도 정렬`,a&&`분류별 보기`,s&&`새로고침해도 유지`].filter(Boolean);return{html:v({title:o,themeId:e.theme,body:l,script:d}),summary:`할 일 목록을 만들었어요. ${f.length?f.join(` · `)+` 기능이 들어갔습니다.`:`제목만 담는 가장 단순한 형태입니다.`}`}}function b(e){let t=e.title||`퀴즈`,n=(e.quizTopic||``).trim(),r=e.quizTimer&&e.quizTimer!==`none`?Number(e.quizTimer):0,i=e.save===`yes`,a=`  <header style="display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;margin-bottom:20px">
    <div style="margin-right:auto">
      <h1>${p(t)}</h1>
      <p class="sub" style="margin:0">${n?p(n)+` 퀴즈`:`나만의 퀴즈`}</p>
    </div>
...
${r?`    <div class="bar" style="margin-bottom:16px">
...
`:``}    <div class="card">
  ...
  <section id="edit" hidden>
    ...
    <div id="qlist"></div>
${i?``:`    <p class="sub" style="margin-top:16px;font-size:12px;color:#f59e0b">
      ⚠ 저장을 끄고 만들어서 새로고침하면 문제가 사라집니다.
      <b>파일로 내보내기</b>로 보관해 두세요.
    </p>
`}  </section>`,o=`${g(i,`quiz.`+(e.title||`default`))}
...
resetForm();
go(questions.length ? 'play' : 'edit');`,s=[`앱 안에서 문제를 만들고 고칠 수 있고`,r?`문제당 ${r}초 제한이 있으며`:`시간 제한 없이 풀 수 있고`,`틀린 문제만 골라 다시 풀 수 있습니다`];return{html:v({title:t,themeId:e.theme,body:a,script:o,extraCss:`.bar { height: 6px; background: var(--border); border-radius: 99px; overflow: hidden; }
...
.crow input[type=radio] { width: 18px; height: 18px; flex: none; accent-color: var(--accent); }`}),summary:`${n?n+` `:``}퀴즈를 만들었어요. ${s.join(`, `)}. 코드를 열 필요 없이 [문제 만들기] 화면에서 바로 추가하시면 되고, 만든 문제는 파일로 내보내 보관할 수 있어요.`}}function x(e){let t=e.title||`집중 타이머`,n=Number(e.timerFocus||25),r=Number(e.timerBreak||5),i=e.save===`yes`,a=`  <h1>${p(t)}</h1>
  <p class="sub">집중 ${n}분 → 휴식 ${r}분을 저절로 오갑니다.</p>
  ...
  <p class="sub" id="done" style="text-align:center;margin-top:18px"></p>`,o=`var FOCUS = ${n} * 60;      // 집중 (초)
var REST = ${r} * 60;       // 휴식 (초)

${g(i,`timer.`+(e.title||`default`))}
...
paint();`;return{html:v({title:t,themeId:e.theme,body:a,script:o}),summary:`집중 ${n}분 · 휴식 ${r}분 타이머를 만들었어요. 두 단계를 저절로 오가고, 끝날 때 알림음이 납니다${i?`. 오늘 몇 번 집중했는지도 남습니다`:``}.`}}var ee=[`about`,`features`,`gallery`,`contact`];function S(e){let t=e.title||`우리 소개`,n=Array.isArray(e.landingSections)?e.landingSections:[],r=ee.filter(e=>n.includes(e)),i=r.map(e=>te[e]()).join(`

`),a=`  <header style="padding:44px 0 32px;text-align:center">
    <h1 style="font-size:38px;margin-bottom:10px">${p(t)}</h1>
...
${i||`    <section class="card">
  ...
  <footer style="margin-top:56px;padding-top:20px;border-top:1px solid var(--border);
    ...
    © ${new Date().getFullYear()} ${p(t)}
  </footer>`,o={about:`소개`,features:`특징`,gallery:`갤러리`,contact:`연락처`};return{html:v({title:t,themeId:e.theme,body:a,script:`var reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
...
}`}),summary:r.length?`소개 페이지를 만들었어요. ${r.map(e=>o[e]).join(` · `)} 순서로 넣었고, 스크롤하면 각 칸이 떠오릅니다.`:`소개 페이지 뼈대를 만들었어요. 내용은 직접 채우시면 됩니다.`}}var C=e=>`    <section class="card" style="margin-bottom:16px">
${e}
    </section>`,te={about:()=>C(`      <h2 style="margin-top:0">소개</h2>
      <p style="color:var(--muted);margin-bottom:0">
        여기에 자세한 이야기를 적으세요. 어떻게 시작했는지, 무엇을 가장 잘하는지처럼
        한 줄 소개에서 못 다한 말을 담는 자리입니다.
      </p>`),features:()=>C(`      <h2 style="margin-top:0">특징</h2>
      <div style="display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(160px,1fr))">
${[[`⚡`,`빠릅니다`,`무엇이 빠른지 적으세요.`],[`🎯`,`정확합니다`,`무엇이 정확한지 적으세요.`],[`💛`,`친절합니다`,`어떻게 친절한지 적으세요.`]].map(([e,t,n])=>`        <div style="background:var(--soft);border-radius:12px;padding:16px">
          <div style="font-size:26px">${e}</div>
          <b>${t}</b>
          <p style="color:var(--muted);font-size:13px;margin:4px 0 0">${n}</p>
        </div>`).join(`
`)}
      </div>`),gallery:()=>C(`      <h2 style="margin-top:0">갤러리</h2>
...
${[1,2,3,4].map(e=>`        <div style="aspect-ratio:4/3;background:var(--soft);border-radius:12px;
          display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:13px">
          사진 ${e}
        </div>`).join(`
`)}
      </div>`),contact:()=>C(`      <h2 style="margin-top:0">연락처</h2>
      ...
      </div>`)},ne={easy:[[`apple`,`사과`,`She ate a red ___ after lunch.`],[`book`,`책`,`I borrowed a ___ from the library.`],[`water`,`물`,`Please drink more ___ every day.`],[`happy`,`행복한`,`The children look very ___ today.`],[`house`,`집`,`They bought a small ___ near the park.`],[`school`,`학교`,`My sister walks to ___ every morning.`],[`friend`,`친구`,`He is my closest ___ from childhood.`],[`family`,`가족`,`Our whole ___ gathers every winter.`],[`market`,`시장`,`We buy fresh fish at the ___ .`],[`garden`,`정원`,`Roses bloom in her small ___ .`],[`animal`,`동물`,`The lion is a wild ___ .`],[`flower`,`꽃`,`He gave her a yellow ___ .`],[`summer`,`여름`,`We swim in the sea every ___ .`],[`winter`,`겨울`,`It snows a lot in ___ .`],[`spring`,`봄`,`Trees turn green in ___ .`],[`morning`,`아침`,`I drink tea every ___ .`],[`night`,`밤`,`The stars shine at ___ .`],[`kitchen`,`부엌`,`Mother is cooking in the ___ .`],[`bedroom`,`침실`,`My ___ has a large window.`],[`window`,`창문`,`Please open the ___ for fresh air.`],[`table`,`탁자`,`Put the plates on the ___ .`],[`chair`,`의자`,`He sat on a wooden ___ .`],[`phone`,`전화기`,`My ___ battery died this morning.`],[`letter`,`편지`,`She wrote a long ___ to her father.`],[`music`,`음악`,`I listen to ___ while studying.`],[`dance`,`춤추다`,`They love to ___ at weddings.`],[`color`,`색깔`,`Blue is my favorite ___ .`],[`bread`,`빵`,`He baked fresh ___ this morning.`],[`cheese`,`치즈`,`This pizza has too much ___ .`],[`coffee`,`커피`,`She drinks black ___ without sugar.`],[`sugar`,`설탕`,`Add two spoons of ___ .`],[`salt`,`소금`,`The soup needs more ___ .`],[`pepper`,`후추`,`Sprinkle some ___ on the eggs.`],[`dinner`,`저녁 식사`,`We eat ___ together at seven.`],[`lunch`,`점심`,`I had a quick ___ at my desk.`],[`plate`,`접시`,`He dropped the glass ___ .`],[`spoon`,`숟가락`,`Stir the soup with a ___ .`],[`knife`,`칼`,`Cut the bread with a sharp ___ .`],[`glass`,`유리잔`,`Pour the juice into a ___ .`],[`bottle`,`병`,`She carried a ___ of water.`],[`basket`,`바구니`,`The ___ is full of apples.`],[`money`,`돈`,`He saved ___ for a new bike.`],[`price`,`가격`,`The ___ of rice went up.`],[`store`,`가게`,`That ___ opens at nine.`],[`street`,`거리`,`Cars filled the narrow ___ .`],[`city`,`도시`,`Seoul is a busy ___ .`],[`town`,`마을`,`She grew up in a quiet ___ .`],[`river`,`강`,`A long ___ runs through the valley.`],[`ocean`,`바다`,`The ___ looks calm today.`],[`beach`,`해변`,`We walked along the sandy ___ .`],[`forest`,`숲`,`Many birds live in this ___ .`],[`hill`,`언덕`,`They climbed the green ___ .`],[`field`,`들판`,`Cows graze in the open ___ .`],[`island`,`섬`,`We took a boat to the ___ .`],[`bridge`,`다리`,`The old ___ crosses the river.`],[`train`,`기차`,`The ___ leaves at six sharp.`],[`plane`,`비행기`,`Our ___ landed on time.`],[`ship`,`배`,`The ___ sailed at dawn.`],[`travel`,`여행하다`,`They ___ abroad every summer.`],[`ticket`,`표`,`I lost my concert ___ .`],[`hotel`,`호텔`,`We stayed at a small ___ .`],[`sleep`,`자다`,`Babies ___ most of the day.`],[`dream`,`꿈`,`She had a strange ___ last night.`],[`walk`,`걷다`,`Let us ___ to the station.`],[`jump`,`뛰다`,`The cat can ___ very high.`],[`swim`,`수영하다`,`He learned to ___ at age five.`],[`climb`,`오르다`,`They plan to ___ that mountain.`],[`throw`,`던지다`,`Do not ___ stones at birds.`],[`catch`,`잡다`,`Try to ___ the ball with both hands.`],[`carry`,`나르다`,`Can you ___ this heavy box?`],[`push`,`밀다`,`Please ___ the door gently.`],[`pull`,`당기다`,`You must ___ the handle hard.`],[`open`,`열다`,`Could you ___ the window?`],[`close`,`닫다`,`Remember to ___ the gate.`],[`break`,`부수다`,`Be careful not to ___ the vase.`],[`build`,`짓다`,`They will ___ a new library.`],[`clean`,`청소하다`,`I ___ my room every Saturday.`],[`wash`,`씻다`,`Please ___ your hands first.`],[`cook`,`요리하다`,`He likes to ___ Italian food.`],[`drink`,`마시다`,`Do not ___ cold water too fast.`],[`taste`,`맛보다`,`Would you like to ___ this soup?`],[`smell`,`냄새 맡다`,`I can ___ fresh bread.`],[`touch`,`만지다`,`Do not ___ the wet paint.`],[`listen`,`귀 기울이다`,`Please ___ to the teacher.`],[`speak`,`말하다`,`She can ___ three languages.`],[`answer`,`대답하다`,`He did not ___ my question.`],[`write`,`쓰다`,`Please ___ your name here.`],[`draw`,`그리다`,`He can ___ birds very well.`],[`learn`,`배우다`,`Children ___ quickly at that age.`],[`teach`,`가르치다`,`She will ___ math next year.`],[`study`,`공부하다`,`They ___ together every evening.`],[`think`,`생각하다`,`Let me ___ about it tonight.`],[`forget`,`잊다`,`Do not ___ your umbrella.`],[`hope`,`바라다`,`We ___ the weather stays clear.`],[`want`,`원하다`,`What do you ___ for dinner?`],[`love`,`사랑하다`,`They ___ their grandchildren dearly.`],[`laugh`,`웃다`,`His jokes always make us ___ .`],[`smile`,`미소짓다`,`She began to ___ warmly.`],[`shout`,`소리치다`,`Do not ___ in the library.`],[`help`,`돕다`,`Can you ___ me carry this?`],[`share`,`나누다`,`Please ___ the candy with your sister.`],[`bring`,`가져오다`,`Do not forget to ___ your notebook.`],[`send`,`보내다`,`I will ___ the file tonight.`],[`sell`,`팔다`,`They ___ vegetables at the corner.`],[`count`,`세다`,`Please ___ the chairs again.`],[`number`,`숫자`,`Write your phone ___ here.`],[`week`,`주`,`I will visit you next ___ .`],[`month`,`달`,`Rent is due every ___ .`],[`today`,`오늘`,`The test is ___ at noon.`],[`hour`,`시간`,`We waited for an ___ in the rain.`],[`minute`,`분`,`Wait one ___ , please.`],[`clock`,`시계`,`The ___ on the wall stopped.`],[`early`,`이른`,`She woke up ___ this morning.`],[`late`,`늦은`,`He was ___ for the meeting.`],[`always`,`항상`,`He ___ arrives before nine.`],[`never`,`결코 ~않다`,`She ___ eats meat.`],[`often`,`자주`,`We ___ meet on Fridays.`],[`quick`,`빠른`,`He gave a ___ answer.`],[`strong`,`강한`,`The wind was very ___ .`],[`weak`,`약한`,`The signal here is too ___ .`],[`brave`,`용감한`,`The ___ boy saved the puppy.`],[`afraid`,`두려운`,`She is ___ of tall places.`],[`lonely`,`외로운`,`He felt ___ in the new city.`],[`proud`,`자랑스러운`,`Her parents are very ___ .`],[`polite`,`예의 바른`,`He is always ___ to guests.`],[`rude`,`무례한`,`That comment was quite ___ .`],[`honest`,`정직한`,`She gave an ___ opinion.`],[`clever`,`영리한`,`That was a ___ solution.`],[`simple`,`단순한`,`The recipe is very ___ .`],[`famous`,`유명한`,`He is a ___ painter.`],[`modern`,`현대의`,`They live in a ___ apartment.`],[`ancient`,`고대의`,`We visited an ___ temple.`],[`future`,`미래`,`Nobody knows the ___ .`],[`past`,`과거`,`Do not dwell on the ___ .`],[`begin`,`시작하다`,`The class will ___ shortly.`],[`finish`,`끝내다`,`Please ___ your homework first.`],[`change`,`바꾸다`,`We must ___ the schedule.`],[`choose`,`고르다`,`You may ___ any seat.`],[`decide`,`결정하다`,`They will ___ tomorrow.`],[`fail`,`실패하다`,`Do not be afraid to ___ .`],[`leave`,`떠나다`,`We ___ for Busan at dawn.`],[`arrive`,`도착하다`,`They will ___ before noon.`],[`return`,`돌아오다`,`She will ___ next Monday.`],[`follow`,`따라가다`,`Please ___ the guide closely.`],[`visit`,`방문하다`,`We ___ our grandmother weekly.`],[`invite`,`초대하다`,`They will ___ all the neighbors.`],[`thank`,`감사하다`,`I want to ___ you sincerely.`],[`sorry`,`미안한`,`He looked truly ___ .`],[`doctor`,`의사`,`The ___ examined my throat.`],[`nurse`,`간호사`,`A kind ___ helped the patient.`],[`farmer`,`농부`,`The ___ grows rice and beans.`],[`artist`,`예술가`,`That ___ paints beautiful skies.`],[`writer`,`작가`,`She is a young ___ .`],[`singer`,`가수`,`The ___ has a lovely voice.`],[`driver`,`운전사`,`The bus ___ waved at us.`],[`police`,`경찰`,`The ___ arrived within minutes.`],[`office`,`사무실`,`His ___ is on the third floor.`],[`company`,`회사`,`She works for a small ___ .`],[`job`,`직업`,`He found a new ___ downtown.`],[`work`,`일하다`,`They ___ from home on Fridays.`],[`meeting`,`회의`,`The ___ starts at ten.`],[`report`,`보고서`,`Please submit your ___ today.`],[`plan`,`계획`,`We made a careful ___ .`],[`idea`,`생각`,`That is a wonderful ___ .`],[`problem`,`문제`,`We solved the ___ together.`],[`reason`,`이유`,`Give me one good ___ .`],[`result`,`결과`,`The ___ surprised everyone.`],[`story`,`이야기`,`Grandmother told a long ___ .`],[`news`,`소식`,`The ___ spread quickly.`],[`picture`,`그림`,`Hang the ___ above the sofa.`],[`film`,`영화`,`That ___ won many awards.`],[`game`,`게임`,`The ___ lasted three hours.`],[`sport`,`운동`,`Swimming is a healthy ___ .`],[`team`,`팀`,`Our ___ won the final match.`],[`player`,`선수`,`The young ___ scored twice.`],[`health`,`건강`,`Sleep is vital for good ___ .`],[`body`,`몸`,`Exercise keeps the ___ strong.`],[`heart`,`심장`,`His ___ beat quickly.`],[`hand`,`손`,`She raised her right ___ .`],[`voice`,`목소리`,`Her ___ was soft and clear.`],[`face`,`얼굴`,`A smile lit up his ___ .`],[`weather`,`날씨`,`The ___ turned cold suddenly.`],[`cloud`,`구름`,`A dark ___ covered the sun.`],[`storm`,`폭풍`,`The ___ broke many branches.`],[`light`,`빛`,`The ___ from the lamp was dim.`],[`shadow`,`그림자`,`His ___ stretched across the road.`],[`fire`,`불`,`They sat around the warm ___ .`],[`stone`,`돌`,`He picked up a smooth ___ .`],[`metal`,`금속`,`The gate is made of ___ .`],[`paper`,`종이`,`Write it on this ___ .`],[`wood`,`나무`,`The table is solid ___ .`],[`jacket`,`재킷`,`Wear a warm ___ outside.`],[`shoes`,`신발`,`These ___ are too tight.`],[`pocket`,`주머니`,`The key is in my ___ .`],[`mirror`,`거울`,`She looked in the ___ .`],[`candle`,`초`,`A single ___ lit the room.`],[`blanket`,`담요`,`Pull the ___ over you.`],[`pillow`,`베개`,`This ___ is very soft.`],[`ladder`,`사다리`,`He climbed the wooden ___ .`],[`hammer`,`망치`,`Pass me the heavy ___ .`]],normal:[[`breakfast`,`아침 식사`,`Never skip ___ before a long day.`],[`mountain`,`산`,`They hiked up the steep ___ .`],[`remember`,`기억하다`,`I still ___ her kind words.`],[`tomorrow`,`내일`,`The results arrive ___ morning.`],[`beautiful`,`아름다운`,`The sunset was truly ___ .`],[`dangerous`,`위험한`,`That road is ___ at night.`],[`difficult`,`어려운`,`The exam was quite ___ .`],[`important`,`중요한`,`Rest is ___ for recovery.`],[`necessary`,`필요한`,`A visa is ___ for that trip.`],[`possible`,`가능한`,`It is ___ to finish today.`],[`knowledge`,`지식`,`She shared her ___ generously.`],[`education`,`교육`,`Good ___ opens many doors.`],[`community`,`공동체`,`The ___ raised funds quickly.`],[`agreement`,`합의`,`They reached an ___ at last.`],[`argument`,`논쟁`,`The ___ lasted all evening.`],[`decision`,`결정`,`It was a difficult ___ .`],[`direction`,`방향`,`We walked in the wrong ___ .`],[`election`,`선거`,`The ___ was held peacefully.`],[`emotion`,`감정`,`His voice showed real ___ .`],[`attention`,`주의`,`Please pay close ___ .`],[`solution`,`해결책`,`We found a simple ___ .`],[`question`,`질문`,`That is an excellent ___ .`],[`position`,`위치`,`Move the lamp to that ___ .`],[`condition`,`상태`,`The car is in fine ___ .`],[`situation`,`상황`,`The ___ improved slowly.`],[`tradition`,`전통`,`This ___ dates back centuries.`],[`culture`,`문화`,`Food reveals a lot about ___ .`],[`history`,`역사`,`She teaches Korean ___ .`],[`science`,`과학`,`Modern ___ advances rapidly.`],[`research`,`연구`,`His ___ took five years.`],[`evidence`,`증거`,`The ___ was convincing.`],[`theory`,`이론`,`That ___ is widely accepted.`],[`method`,`방법`,`We tried a different ___ .`],[`process`,`과정`,`The ___ takes several weeks.`],[`progress`,`진전`,`She made steady ___ .`],[`purpose`,`목적`,`What is the ___ of this rule?`],[`benefit`,`이익`,`Exercise has a clear ___ .`],[`advantage`,`장점`,`Speed is our main ___ .`],[`disadvantage`,`단점`,`Cost is the main ___ .`],[`challenge`,`도전`,`The project was a real ___ .`],[`opinion`,`의견`,`I respect your ___ .`],[`suggestion`,`제안`,`Thank you for the ___ .`],[`statement`,`진술`,`He signed the written ___ .`],[`announcement`,`발표`,`The ___ came this morning.`],[`conversation`,`대화`,`We had a long ___ .`],[`discussion`,`토론`,`The ___ grew heated.`],[`description`,`묘사`,`Her ___ was very vivid.`],[`instruction`,`지시`,`Follow each ___ carefully.`],[`requirement`,`요건`,`Experience is a strict ___ .`],[`equipment`,`장비`,`The ___ arrived yesterday.`],[`material`,`재료`,`Choose a lighter ___ .`],[`product`,`제품`,`This ___ sells very well.`],[`industry`,`산업`,`The film ___ is growing.`],[`business`,`사업`,`Their ___ expanded abroad.`],[`customer`,`고객`,`Every ___ deserves respect.`],[`service`,`서비스`,`The ___ here is excellent.`],[`quality`,`품질`,`We never compromise on ___ .`],[`quantity`,`양`,`Order a smaller ___ .`],[`increase`,`증가하다`,`Prices will ___ next month.`],[`decrease`,`감소하다`,`Sales began to ___ slowly.`],[`develop`,`개발하다`,`They ___ new software.`],[`improve`,`개선하다`,`She wants to ___ her writing.`],[`provide`,`제공하다`,`We ___ free training.`],[`produce`,`생산하다`,`This farm can ___ enough rice.`],[`consider`,`고려하다`,`Please ___ my request.`],[`continue`,`계속하다`,`They will ___ the search.`],[`complete`,`완성하다`,`Please ___ the form.`],[`prepare`,`준비하다`,`We must ___ for the storm.`],[`discover`,`발견하다`,`Scientists ___ new species yearly.`],[`describe`,`묘사하다`,`Can you ___ the man?`],[`explain`,`설명하다`,`Let me ___ the rule again.`],[`express`,`표현하다`,`She struggled to ___ her feelings.`],[`suggest`,`제안하다`,`I ___ we leave early.`],[`recommend`,`추천하다`,`Doctors ___ regular exercise.`],[`announce`,`발표하다`,`They will ___ the winner soon.`],[`achieve`,`달성하다`,`He hopes to ___ his goal.`],[`succeed`,`성공하다`,`She worked hard to ___ .`],[`prevent`,`예방하다`,`Vaccines ___ many illnesses.`],[`protect`,`보호하다`,`Trees ___ the soil.`],[`support`,`지지하다`,`Many people ___ the plan.`],[`require`,`요구하다`,`These plants ___ little water.`],[`include`,`포함하다`,`Does the price ___ tax?`],[`contain`,`담고 있다`,`This box may ___ glass.`],[`reduce`,`줄이다`,`We must ___ our waste.`],[`replace`,`대체하다`,`Please ___ the broken part.`],[`repair`,`수리하다`,`He can ___ old clocks.`],[`measure`,`측정하다`,`Please ___ the room first.`],[`compare`,`비교하다`,`Let us ___ the two plans.`],[`connect`,`연결하다`,`This road will ___ both towns.`],[`separate`,`분리하다`,`Please ___ the paper and plastic.`],[`organize`,`조직하다`,`She will ___ the event.`],[`manage`,`관리하다`,`He can ___ a large team.`],[`control`,`통제하다`,`Try to ___ your temper.`],[`operate`,`운영하다`,`They ___ three factories.`],[`maintain`,`유지하다`,`We must ___ these standards.`],[`establish`,`설립하다`,`They plan to ___ a school.`],[`introduce`,`소개하다`,`Let me ___ my colleague.`],[`influence`,`영향`,`Her ___ on the team was huge.`],[`pressure`,`압력`,`He works well under ___ .`],[`balance`,`균형`,`Find a ___ between work and rest.`],[`distance`,`거리`,`The ___ is about ten kilometers.`],[`surface`,`표면`,`The ___ felt rough.`],[`structure`,`구조`,`The ___ needs repair.`],[`pattern`,`무늬`,`The cloth has a floral ___ .`],[`feature`,`특징`,`Its best ___ is the screen.`],[`element`,`요소`,`Trust is a key ___ .`],[`factor`,`요인`,`Cost was the deciding ___ .`],[`effect`,`효과`,`The medicine had a quick ___ .`],[`impact`,`충격`,`The news had a strong ___ .`],[`growth`,`성장`,`Steady ___ continued all year.`],[`success`,`성공`,`Her ___ inspired many.`],[`failure`,`실패`,`The ___ taught us a lot.`],[`mistake`,`실수`,`Everyone makes a ___ sometimes.`],[`accident`,`사고`,`The ___ blocked the road.`],[`danger`,`위험`,`The sign warns of ___ .`],[`safety`,`안전`,`Wear a helmet for your ___ .`],[`risk`,`위험 부담`,`Every choice carries some ___ .`],[`chance`,`기회`,`Give him another ___ .`],[`opportunity`,`기회`,`This is a rare ___ .`],[`ability`,`능력`,`She has real ___ in math.`],[`skill`,`기술`,`Cooking is a useful ___ .`],[`talent`,`재능`,`He shows great musical ___ .`],[`effort`,`노력`,`Your ___ finally paid off.`],[`practice`,`연습`,`Daily ___ makes a difference.`],[`experience`,`경험`,`Her ___ helped the team.`],[`memory`,`기억`,`That song brings back a ___ .`],[`attitude`,`태도`,`His ___ improved quickly.`],[`behavior`,`행동`,`Her ___ surprised us.`],[`character`,`성격`,`He has a gentle ___ .`],[`personal`,`개인의`,`That is a ___ matter.`],[`private`,`사적인`,`This is a ___ conversation.`],[`public`,`공공의`,`The park is open to the ___ .`],[`general`,`일반적인`,`That is the ___ opinion.`],[`special`,`특별한`,`Today is a ___ occasion.`],[`similar`,`비슷한`,`The twins have ___ voices.`],[`different`,`다른`,`Their views are quite ___ .`],[`various`,`다양한`,`We tried ___ approaches.`],[`certain`,`확실한`,`I am ___ he will come.`],[`obvious`,`명백한`,`The answer was ___ .`],[`complex`,`복잡한`,`The rules are rather ___ .`],[`serious`,`심각한`,`This is a ___ problem.`],[`careful`,`조심스러운`,`Be ___ on the wet floor.`],[`popular`,`인기 있는`,`That song became ___ fast.`],[`natural`,`자연의`,`This soap uses ___ oils.`],[`physical`,`신체의`,`Regular ___ activity helps.`],[`mental`,`정신의`,`Rest supports ___ health.`],[`social`,`사회의`,`They study ___ change.`],[`economic`,`경제의`,`The ___ outlook improved.`],[`political`,`정치의`,`It became a ___ issue.`],[`medical`,`의료의`,`He needs ___ attention.`],[`legal`,`법적인`,`She sought ___ advice.`],[`financial`,`재정의`,`The ___ report is due.`],[`available`,`이용 가능한`,`No seats are ___ tonight.`],[`suitable`,`적합한`,`This film is not ___ for children.`],[`reliable`,`믿을 만한`,`He is a ___ partner.`],[`flexible`,`유연한`,`Our schedule is ___ .`],[`efficient`,`효율적인`,`The new system is more ___ .`],[`effective`,`효과적인`,`That method proved ___ .`],[`accurate`,`정확한`,`Her measurements were ___ .`],[`precise`,`정밀한`,`We need a ___ figure.`],[`approximate`,`대략의`,`Give me an ___ number.`],[`average`,`평균의`,`His score was above ___ .`],[`maximum`,`최대의`,`The ___ speed is eighty.`],[`minimum`,`최소의`,`There is a ___ order.`],[`recent`,`최근의`,`A ___ study confirmed it.`],[`current`,`현재의`,`The ___ rate is low.`],[`previous`,`이전의`,`Check the ___ chapter.`],[`following`,`다음의`,`Read the ___ passage.`],[`further`,`더 이상의`,`No ___ action is needed.`],[`whole`,`전체의`,`She read the ___ book.`],[`entire`,`전체의`,`The ___ village celebrated.`],[`several`,`여럿의`,`He asked ___ questions.`],[`multiple`,`다수의`,`There were ___ reasons.`],[`single`,`단 하나의`,`Not a ___ light was on.`],[`double`,`두 배의`,`Order a ___ portion.`],[`exact`,`정확한`,`Tell me the ___ time.`],[`final`,`마지막의`,`This is the ___ round.`],[`initial`,`처음의`,`Our ___ plan failed.`],[`original`,`원래의`,`The ___ copy is missing.`],[`additional`,`추가의`,`We need ___ chairs.`],[`sufficient`,`충분한`,`Bring ___ water.`],[`adequate`,`적당한`,`The lighting is ___ .`],[`constant`,`끊임없는`,`There was ___ noise outside.`],[`frequent`,`잦은`,`He is a ___ visitor.`],[`occasional`,`가끔의`,`We take ___ breaks.`],[`temporary`,`일시적인`,`This is a ___ solution.`],[`permanent`,`영구적인`,`She found a ___ job.`],[`ordinary`,`평범한`,`It was an ___ Tuesday.`],[`unusual`,`흔치 않은`,`That is an ___ color.`],[`familiar`,`익숙한`,`The tune sounded ___ .`],[`foreign`,`외국의`,`She studies a ___ language.`],[`distant`,`먼`,`They come from a ___ island.`],[`nearby`,`가까운`,`Is there a ___ pharmacy?`],[`central`,`중심의`,`The ___ office moved.`],[`northern`,`북쪽의`,`The ___ region is colder.`],[`southern`,`남쪽의`,`The ___ coast is warmer.`],[`eastern`,`동쪽의`,`The ___ sky brightened.`],[`western`,`서쪽의`,`They drove to the ___ border.`],[`vegetable`,`채소`,`Eat one green ___ each day.`],[`furniture`,`가구`,`They bought new ___ for the flat.`]],hard:[[`responsibility`,`책임`,`Taking ___ is part of growing up.`],[`extraordinary`,`비범한`,`She showed ___ courage.`],[`international`,`국제의`,`They signed an ___ treaty.`],[`understanding`,`이해`,`We reached a mutual ___ .`],[`consideration`,`고려`,`Please give it careful ___ .`],[`recommendation`,`추천`,`His ___ helped me greatly.`],[`characteristic`,`특징`,`Patience is her main ___ .`],[`representative`,`대표`,`A union ___ spoke first.`],[`administration`,`행정`,`The new ___ took office.`],[`communication`,`의사소통`,`Clear ___ prevents mistakes.`],[`determination`,`결단력`,`Her ___ impressed everyone.`],[`investigation`,`조사`,`The ___ lasted six months.`],[`organization`,`조직`,`The ___ helps refugees.`],[`performance`,`성과`,`His ___ improved steadily.`],[`relationship`,`관계`,`Their ___ grew stronger.`],[`significant`,`중대한`,`That was a ___ discovery.`],[`sustainable`,`지속 가능한`,`We need ___ energy sources.`],[`sophisticated`,`정교한`,`The device is highly ___ .`],[`comprehensive`,`포괄적인`,`They wrote a ___ report.`],[`controversial`,`논란이 많은`,`The decision was ___ .`],[`demonstration`,`시연`,`The ___ drew a large crowd.`],[`environmental`,`환경의`,`They study ___ damage.`],[`fundamental`,`근본적인`,`This is a ___ principle.`],[`independence`,`독립`,`The colony gained ___ .`],[`intelligence`,`지능`,`Artificial ___ is advancing.`],[`nevertheless`,`그럼에도`,`It rained; ___ , we went.`],[`overwhelming`,`압도적인`,`The response was ___ .`],[`participation`,`참여`,`Student ___ was high.`],[`philosophical`,`철학적인`,`He raised a ___ question.`],[`preliminary`,`예비의`,`These are ___ results.`],[`psychological`,`심리적인`,`The strain was ___ .`],[`qualification`,`자격`,`She has every ___ needed.`],[`recognition`,`인정`,`His work finally gained ___ .`],[`remarkable`,`놀라운`,`It was a ___ recovery.`],[`revolutionary`,`혁명적인`,`The idea was ___ .`],[`spectacular`,`장관인`,`The view was ___ .`],[`technological`,`기술의`,`___ progress never stops.`],[`transformation`,`변화`,`The ___ took a decade.`],[`unfortunately`,`불행히도`,`___ , the flight was canceled.`],[`accommodation`,`숙박 시설`,`The ___ was clean and quiet.`],[`accomplishment`,`업적`,`That was a real ___ .`],[`advertisement`,`광고`,`The ___ ran for a month.`],[`appreciation`,`감사`,`She expressed deep ___ .`],[`architecture`,`건축`,`Roman ___ still stands.`],[`circumstance`,`상황`,`Under no ___ should you enter.`],[`collaboration`,`협력`,`The ___ produced great work.`],[`competition`,`경쟁`,`The ___ was fierce.`],[`concentration`,`집중`,`The task requires ___ .`],[`consequence`,`결과`,`He faced the ___ calmly.`],[`constitution`,`헌법`,`The ___ protects free speech.`],[`contribution`,`기여`,`Her ___ was invaluable.`],[`cooperation`,`협조`,`Thank you for your ___ .`],[`corporation`,`기업`,`A large ___ bought the land.`],[`correspondence`,`서신`,`Their ___ lasted years.`],[`disappointed`,`실망한`,`She looked deeply ___ .`],[`discrimination`,`차별`,`The law bans ___ .`],[`distribution`,`분배`,`The ___ was uneven.`],[`documentation`,`문서`,`Bring all the ___ .`],[`effectiveness`,`효과`,`They measured its ___ .`],[`encouragement`,`격려`,`Her ___ kept me going.`],[`entertainment`,`오락`,`The ___ lasted all evening.`],[`enthusiasm`,`열정`,`He spoke with ___ .`],[`environment`,`환경`,`We must protect the ___ .`],[`establishment`,`설립`,`The ___ of the fund took years.`],[`examination`,`시험`,`The ___ begins at nine.`],[`expectation`,`기대`,`The result exceeded every ___ .`],[`explanation`,`설명`,`His ___ made sense.`],[`exploration`,`탐험`,`Space ___ continues.`],[`imagination`,`상상력`,`Children have vivid ___ .`],[`implementation`,`시행`,`The ___ starts in May.`],[`improvement`,`개선`,`There is room for ___ .`],[`independent`,`독립적인`,`She became financially ___ .`],[`individual`,`개인`,`Each ___ has a role.`],[`industrial`,`산업의`,`The ___ zone expanded.`],[`infrastructure`,`기반 시설`,`The ___ needs investment.`],[`initiative`,`주도권`,`He took the ___ himself.`],[`innovation`,`혁신`,`___ drives their success.`],[`inspiration`,`영감`,`Nature was her ___ .`],[`institution`,`기관`,`The ___ was founded in 1890.`],[`interaction`,`상호작용`,`Social ___ matters to children.`],[`interpretation`,`해석`,`Her ___ differed from mine.`],[`introduction`,`소개`,`The ___ was rather long.`],[`investment`,`투자`,`The ___ paid off well.`],[`legislation`,`법률`,`New ___ passed last month.`],[`maintenance`,`유지 보수`,`Regular ___ prevents damage.`],[`manufacture`,`제조하다`,`They ___ medical devices.`],[`negotiation`,`협상`,`The ___ lasted all night.`],[`observation`,`관찰`,`That is a sharp ___ .`],[`occupation`,`직업`,`State your ___ here.`],[`perspective`,`관점`,`Try another ___ .`],[`possibility`,`가능성`,`There is a slight ___ .`],[`preparation`,`준비`,`The ___ took weeks.`],[`presentation`,`발표`,`Her ___ was clear.`],[`production`,`생산`,`Steel ___ increased.`],[`profession`,`직업`,`Teaching is a noble ___ .`],[`protection`,`보호`,`Wear gloves for ___ .`],[`publication`,`출판`,`The ___ date moved.`],[`reflection`,`반영`,`Sales are a ___ of quality.`],[`regulation`,`규정`,`Every ___ must be followed.`],[`reputation`,`평판`,`The shop has a good ___ .`],[`resistance`,`저항`,`They met strong ___ .`],[`resolution`,`결의`,`The council passed a ___ .`],[`satisfaction`,`만족`,`She smiled with ___ .`],[`significance`,`중요성`,`Few grasped its ___ .`],[`specialist`,`전문가`,`Ask a ___ about it.`],[`statistics`,`통계`,`The ___ tell a clear story.`],[`supervision`,`감독`,`Work under close ___ .`],[`technology`,`기술`,`Modern ___ changed everything.`],[`temperature`,`온도`,`The ___ dropped sharply.`],[`university`,`대학교`,`He entered ___ last year.`],[`acknowledge`,`인정하다`,`Please ___ receipt of this.`],[`accumulate`,`축적하다`,`Dust tends to ___ here.`],[`anticipate`,`예상하다`,`We ___ heavy traffic.`],[`appreciate`,`감사하다`,`I truly ___ your help.`],[`approximately`,`대략`,`It costs ___ fifty dollars.`],[`assumption`,`가정`,`That ___ proved wrong.`],[`collaborate`,`협력하다`,`The two teams will ___ .`],[`commitment`,`헌신`,`Her ___ never wavered.`],[`compensate`,`보상하다`,`They will ___ the loss.`],[`competitive`,`경쟁적인`,`The market is very ___ .`],[`comprehend`,`이해하다`,`It is hard to ___ the scale.`],[`concentrate`,`집중하다`,`Please ___ on the task.`],[`confidence`,`자신감`,`She spoke with ___ .`],[`confirmation`,`확인`,`We await written ___ .`],[`consequently`,`결과적으로`,`It rained; ___ , we stayed.`],[`considerable`,`상당한`,`It required ___ effort.`],[`consistent`,`일관된`,`His results are ___ .`],[`constructive`,`건설적인`,`She gave ___ feedback.`],[`contemporary`,`현대의`,`This is ___ music.`],[`controversy`,`논란`,`The film caused ___ .`],[`conventional`,`관습적인`,`They chose a ___ approach.`],[`convincing`,`설득력 있는`,`His argument was ___ .`],[`coordination`,`조정`,`The task needs ___ .`],[`definitely`,`분명히`,`I will ___ attend.`],[`dependence`,`의존`,`Reduce your ___ on cars.`],[`desperately`,`필사적으로`,`He searched ___ .`],[`distinguish`,`구별하다`,`Can you ___ the two sounds?`],[`distinctive`,`독특한`,`It has a ___ flavor.`],[`dramatically`,`극적으로`,`Costs rose ___ .`],[`accessibility`,`접근성`,`They improved website ___ .`],[`accountability`,`책임`,`Voters demanded ___ .`],[`essentially`,`본질적으로`,`They are ___ the same.`],[`achievement`,`성취`,`Winning was a huge ___ .`],[`eventually`,`결국`,`She ___ agreed.`],[`exaggerate`,`과장하다`,`Do not ___ the danger.`],[`exceptional`,`뛰어난`,`The service was ___ .`],[`expenditure`,`지출`,`Cut unnecessary ___ .`],[`experiment`,`실험`,`The ___ succeeded.`],[`fascinating`,`매혹적인`,`The lecture was ___ .`],[`frequently`,`자주`,`He travels ___ .`],[`generation`,`세대`,`Each ___ faces new problems.`],[`advancement`,`발전`,`Medical ___ saves lives.`],[`illustrate`,`설명하다`,`Let me ___ with an example.`],[`immediately`,`즉시`,`Call me ___ .`],[`impressive`,`인상적인`,`The result was ___ .`],[`incredible`,`믿기 힘든`,`The story sounds ___ .`],[`inevitable`,`피할 수 없는`,`Change was ___ .`],[`influential`,`영향력 있는`,`She is an ___ critic.`],[`ingredient`,`재료`,`Salt is a key ___ .`],[`inhabitant`,`주민`,`Every ___ was evacuated.`],[`insufficient`,`불충분한`,`The funds were ___ .`],[`intellectual`,`지적인`,`It was an ___ debate.`],[`investigate`,`조사하다`,`Police will ___ the case.`],[`justification`,`정당화`,`There is no ___ for it.`],[`legitimate`,`정당한`,`She raised a ___ concern.`],[`magnificent`,`웅장한`,`The hall was ___ .`],[`manipulate`,`조종하다`,`Do not ___ the data.`],[`measurement`,`측정`,`Take an exact ___ .`],[`mysterious`,`신비한`,`A ___ light appeared.`],[`alternative`,`대안`,`Is there a cheaper ___ ?`],[`nutritious`,`영양가 있는`,`Beans are highly ___ .`],[`obligation`,`의무`,`You have no ___ to answer.`],[`occasionally`,`가끔`,`We meet ___ .`],[`originally`,`원래`,`It was ___ a barn.`],[`outstanding`,`뛰어난`,`Her essay was ___ .`],[`particularly`,`특히`,`It is ___ cold today.`],[`persuasive`,`설득력 있는`,`He gave a ___ speech.`],[`phenomenon`,`현상`,`This ___ puzzles scientists.`],[`photograph`,`사진`,`She framed the old ___ .`],[`population`,`인구`,`The ___ doubled in ten years.`],[`practically`,`사실상`,`The work is ___ done.`],[`prediction`,`예측`,`His ___ proved accurate.`],[`prescription`,`처방전`,`Bring your ___ to the counter.`],[`presumably`,`아마도`,`He is ___ still asleep.`],[`previously`,`이전에`,`We ___ met in Seoul.`],[`prohibited`,`금지된`,`Smoking is strictly ___ .`],[`proportion`,`비율`,`A large ___ agreed.`],[`reasonable`,`합리적인`,`The price seems ___ .`],[`reluctantly`,`마지못해`,`She ___ accepted.`],[`representation`,`대표`,`They demanded fair ___ .`],[`respectively`,`각각`,`They finished first and third, ___ .`],[`restriction`,`제한`,`The ___ was lifted.`],[`simultaneously`,`동시에`,`Both alarms rang ___ .`],[`specifically`,`구체적으로`,`He asked ___ about cost.`],[`substantial`,`상당한`,`They made a ___ profit.`],[`sufficiently`,`충분히`,`The soup is ___ warm.`],[`surprisingly`,`놀랍게도`,`___ , nobody complained.`],[`unexpectedly`,`예기치 않게`,`He arrived ___ early.`],[`consciousness`,`의식`,`He regained ___ after an hour.`],[`contradiction`,`모순`,`Her story contained a ___ .`]]},re=[{id:`easy`,name:`쉬움`,count:10,desc:`7글자 이하 · 10문제`},{id:`normal`,name:`보통`,count:15,desc:`12글자 이하 · 15문제`},{id:`hard`,name:`상급`,count:30,desc:`10글자 이상 · 30문제`}];function w(){let e=e=>e.map(([e,t,n])=>`[${JSON.stringify(e)},${JSON.stringify(t)},${JSON.stringify(n)}]`).join(`,
`);return`var DICT = {
easy: [
${e(ne.easy)}
],
...
};`}function ie(e){let t=e.title||`영어 단어장`,n=e.save===`yes`,r=Array.isArray(e.vocabTypes)?e.vocabTypes.filter(e=>e!==`none`):[],i=r.length?r:[`meaning`,`spelling`,`blank`],a=`  <header style="display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap;margin-bottom:18px">
    <div style="margin-right:auto">
      <h1>${p(t)}</h1>
...
var LEVELS = ${JSON.stringify(re)};
var TYPES = ${JSON.stringify(i)};
var TYPE_INFO = {
...
go('home');`,s={meaning:`스펠링 보고 뜻 맞히기`,spelling:`뜻 보고 스펠링 쓰기`,blank:`문장 빈칸 채우기`};return{html:v({title:t,themeId:e.theme,body:a,script:o,extraCss:`.bar { height: 6px; background: var(--border); border-radius: 99px; overflow: hidden; }
.bar-fill { height: 100%; width: 0; background: var(--accent); border-radius: 99px; transition: width .3s ease; }
.tag {
...
.more { width: 100%; margin-top: 8px; }`}),summary:`영어 단어장을 만들었어요. 단어 600개(초급·중급·상급 각 200개)가 들어 있고, 난이도를 고르면 그 등급에서 무작위로 뽑아 냅니다 (쉬움 10문제 · 보통 15문제 · 상급 30문제). 문제 유형은 ${i.map(e=>s[e]).join(`, `)}이고, 틀린 단어는 오답 노트에 모아 두었다가 모아서 다시 풀 수 있어요. [단어 도감]에서는 등급별로 뜻·스펠링·사용 예를 훑어볼 수 있고, 단어나 뜻으로 찾거나 아직 못 외운 것만 골라 볼 수 있습니다.`}}var ae=[{id:`todo`,name:`할 일 목록`,icon:`✅`,desc:`적고, 지우고, 다 한 것을 표시합니다`,make:y},{id:`quiz`,name:`퀴즈`,icon:`🧠`,desc:`문제를 내고 점수를 매깁니다`,make:b},{id:`timer`,name:`집중 타이머`,icon:`⏱`,desc:`집중과 휴식을 번갈아 재줍니다`,make:x},{id:`vocab`,name:`영어 단어장`,icon:`📖`,desc:`단어 600개로 뜻·스펠링·빈칸 문제를 냅니다`,make:ie},{id:`landing`,name:`소개 페이지`,icon:`🪧`,desc:`나 · 우리 가게 · 동아리를 소개합니다`,make:S}],oe=Object.fromEntries(ae.map(e=>[e.id,e]));function se(e={}){let t=oe[e.kind];if(!t)throw Error(`알 수 없는 종류: ${e.kind}`);let{html:n,summary:r}=t.make(e);return{files:[{path:`${ce(e.title||t.name)}.html`,code:n}],summary:r}}function ce(e){return String(e).replace(/[\\/:*?"<>|]/g,``).replace(/\s+/g,`-`).replace(/^[-.]+|[-.]+$/g,``).slice(0,40)||`my-app`}var le=ae.map(e=>({value:e.id,label:`${e.icon} ${e.name}`,desc:e.desc})),ue=[{value:`violet`,label:`보라`,desc:`차분하고 무난하다`},{value:`blue`,label:`파랑`,desc:`신뢰감 있는 기본색`},{value:`emerald`,label:`초록`,desc:`눈이 편하다`},{value:`amber`,label:`주황`,desc:`활기차다`},{value:`rose`,label:`분홍`,desc:`부드럽다`}],T=(e,t)=>Object.prototype.hasOwnProperty.call(e,t),E=[{id:`kind`,kind:`choice`,text:`안녕하세요! 무엇을 만들어 드릴까요?`,hint:`고르면 그에 맞춰 필요한 것만 더 여쭤볼게요.`,when:()=>!0,options:le},{id:`title`,kind:`text`,text:`이름을 뭐라고 할까요?`,hint:`화면 맨 위에 크게 들어갑니다. 비워두면 기본 이름을 씁니다.`,placeholder:`예: 나의 하루`,when:e=>!!e.kind},{id:`todoFields`,kind:`multi`,text:`할 일 하나에 무엇까지 적을 수 있게 할까요?`,hint:`여러 개 고를 수 있고, 아무것도 안 골라도 됩니다.`,when:e=>e.kind===`todo`,options:[{value:`due`,label:`📅 마감일`,desc:`지난 것은 빨갛게 표시합니다`},{value:`priority`,label:`🔥 중요도`,desc:`높음/보통/낮음 — 높은 것이 위로 옵니다`},{value:`tag`,label:`🏷 분류`,desc:`분류별로 걸러 볼 수 있습니다`},{value:`none`,label:`아니요, 제목만`,desc:`가장 단순한 형태`}]},{id:`quizTopic`,kind:`text`,text:`퀴즈 주제가 무엇인가요?`,hint:`문제는 직접 넣을 수 있게 만들어 드리고, 예시 문제 3개를 미리 채워둡니다.`,placeholder:`예: 한국사, 영어 단어`,when:e=>e.kind===`quiz`},{id:`quizTimer`,kind:`choice`,text:`문제마다 제한 시간을 둘까요?`,when:e=>e.kind===`quiz`,options:[{value:`none`,label:`없음`,desc:`천천히 풀게 합니다`},{value:`10`,label:`10초`,desc:`빠른 순발력 퀴즈`},{value:`30`,label:`30초`,desc:`생각할 틈은 주는 정도`}]},{id:`vocabTypes`,kind:`multi`,text:`어떤 방식으로 물어볼까요?`,hint:`여러 개 고르면 번갈아 나옵니다. 안 고르면 세 가지 다 넣어 드려요.`,when:e=>e.kind===`vocab`,options:[{value:`meaning`,label:`🔤 스펠링 보고 뜻 맞히기`,desc:`보기 네 개 중에서 고릅니다`},{value:`spelling`,label:`⌨️ 뜻 보고 스펠링 쓰기`,desc:`직접 타이핑합니다. 틀린 글자를 짚어줍니다`},{value:`blank`,label:`📝 문장 빈칸 채우기`,desc:`예문에 들어갈 낱말을 고릅니다`}]},{id:`timerFocus`,kind:`choice`,text:`한 번에 몇 분씩 집중할까요?`,when:e=>e.kind===`timer`,options:[{value:`25`,label:`25분`,desc:`뽀모도로 기본값`},{value:`45`,label:`45분`,desc:`한 과목 분량`},{value:`50`,label:`50분`,desc:`수업 한 교시`}]},{id:`timerBreak`,kind:`choice`,text:`쉬는 시간은요?`,when:e=>e.kind===`timer`,options:[{value:`5`,label:`5분`,desc:`짧게 끊고 바로 복귀`},{value:`10`,label:`10분`,desc:`가볍게 걷다 올 정도`},{value:`15`,label:`15분`,desc:`충분히 쉬기`}]},{id:`landingSections`,kind:`multi`,text:`어떤 내용을 넣을까요?`,hint:`고른 순서와 상관없이 읽기 좋은 순서로 배치합니다.`,when:e=>e.kind===`landing`,options:[{value:`about`,label:`소개`,desc:`무엇을 하는 곳인지`},{value:`features`,label:`특징 3가지`,desc:`카드 세 장으로 보여줍니다`},{value:`gallery`,label:`사진 갤러리`,desc:`자리를 잡아 두고 사진만 바꾸면 됩니다`},{value:`contact`,label:`연락처`,desc:`이메일과 링크`}]},{id:`save`,kind:`choice`,text:`새로고침해도 내용이 남아 있어야 할까요?`,hint:`브라우저에 저장합니다. 서버가 없어도 되고, 내 기기에만 남습니다.`,when:e=>!!e.kind&&e.kind!==`landing`,options:[{value:`yes`,label:`네, 남겨주세요`,desc:`창을 닫았다 열어도 그대로입니다`},{value:`no`,label:`아니요`,desc:`새로고침하면 처음부터`}]},{id:`theme`,kind:`choice`,text:`마지막으로, 어떤 색이 좋으세요?`,hint:`밝은 화면과 어두운 화면 둘 다 알아서 맞춰 만듭니다.`,when:e=>!!e.kind,options:ue}];Object.fromEntries(E.map(e=>[e.id,e]));function de(e={}){return E.find(t=>!T(e,t.id)&&t.when(e))||null}var fe={id:`rules`,name:`규칙 기반`,async ask(e){return de(e)},async build(e){return se(e)}},pe=e((e=>{var t=Symbol.for(`react.transitional.element`);function n(e,n,r){var i=null;if(r!==void 0&&(i=``+r),n.key!==void 0&&(i=``+n.key),`key`in n)for(var a in r={},n)a!==`key`&&(r[a]=n[a]);else r=n;return n=r.ref,{$$typeof:t,type:e,key:i,ref:n===void 0?null:n,props:r}}e.jsx=n,e.jsxs=n})),D=e(((e,t)=>{t.exports=pe()}))();function O({question:e,onAnswer:t}){return e.kind===`text`?(0,D.jsx)(he,{q:e,onAnswer:t}):e.kind===`multi`?(0,D.jsx)(me,{q:e,onAnswer:t}):(0,D.jsx)(k,{q:e,onAnswer:t})}function k({q:e,onAnswer:t}){return(0,D.jsx)(`div`,{className:`grid gap-2 sm:grid-cols-2`,children:e.options.map(e=>(0,D.jsxs)(`button`,{onClick:()=>t(e.value),className:`group rounded-2xl border border-white/10 bg-white/[.03] px-4 py-3 text-left
          ...
          text-[14px] font-black text-white transition hover:brightness-110`,children:n.trim()?`보내기`:`건너뛰기`})]})}var ge=(e=[])=>e.find(e=>e.path.endsWith(`.html`))||null;function _e({result:e,onRestart:t}){let[n,r]=(0,l.useState)(`preview`),[i,a]=(0,l.useState)(!1),o=(0,l.useMemo)(()=>ge(e.files),[e.files]);return(0,l.useEffect)(()=>{if(!i)return;let e=setTimeout(()=>a(!1),1600);return()=>clearTimeout(e)},[i]),(0,D.jsxs)(`div`,{className:`flex h-full flex-col`,children:[(0,D.jsxs)(`div`,{className:`flex flex-wrap items-center gap-2 border-b border-white/10 px-4 py-3`,children:[(0,D.jsxs)(`div`,{className:`mr-auto min-w-0`,children:[(0,D.jsx)(`div`,{className:`text-[11px] font-bold tracking-widest text-violet-300`,children:`완성`}),(0,D.jsx)(`div`,{className:`truncate text-[13px] text-zinc-400`,children:o.path})]}),(0,D.jsx)(`div`,{className:`flex rounded-xl border border-white/10 bg-black/40 p-0.5`,children:[[`preview`,`미리보기`],[`code`,`코드`]].map(([e,t])=>(0,D.jsx)(`button`,{onClick:()=>r(e),className:`rounded-lg px-3 py-1.5 text-[12px] font-bold transition ${n===e?`bg-white/10 text-white`:`text-zinc-500 hover:text-zinc-300`}`,children:t},e))}),(0,D.jsx)(`button`,{onClick:()=>{navigator.clipboard?.writeText(o.code),a(!0)},className:`rounded-xl border border-white/10 px-3 py-1.5 text-[12px] font-bold text-zinc-300 transition hover:bg-white/10`,children:i?`복사됨`:`코드 복사`}),(0,D.jsx)(`button`,{onClick:()=>{let e=new Blob([o.code],{type:`text/html;charset=utf-8`}),t=URL.createObjectURL(e),n=document.createElement(`a`);n.href=t,n.download=o.path,n.click(),URL.revokeObjectURL(t)},className:`rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 px-4 py-1.5 text-[12px] font-black text-white transition hover:brightness-110`,children:`내려받기`})]}),(0,D.jsx)(`div`,{className:`min-h-0 flex-1`,children:n===`preview`?(0,D.jsx)(ve,{code:o.code}):(0,D.jsx)(`pre`,{className:`h-full overflow-auto bg-black/40 p-4 text-[11.5px] leading-relaxed text-zinc-300`,children:(0,D.jsx)(`code`,{children:o.code})})}),(0,D.jsxs)(`div`,{className:`border-t border-white/10 px-4 py-3`,children:[(0,D.jsx)(`p`,{className:`text-[12px] leading-relaxed text-zinc-400`,children:e.summary}),(0,D.jsx)(`button`,{onClick:t,className:`mt-3 rounded-xl border border-white/12 px-4 py-2 text-[12px] font-bold text-zinc-300 transition hover:bg-white/10`,children:`다른 것 만들기`})]})]})}function ve({code:e}){let t=(0,l.useRef)(null),[n,r]=(0,l.useState)(0);return(0,l.useEffect)(()=>{r(e=>e+1)},[e]),(0,D.jsxs)(`div`,{className:`relative h-full`,children:[(0,D.jsx)(`iframe`,{ref:t,srcDoc:e,title:`미리보기`,sandbox:`allow-scripts allow-modals`,className:`h-full w-full bg-white`},n),(0,D.jsx)(`button`,{onClick:()=>r(e=>e+1),className:`absolute right-3 top-3 rounded-lg bg-black/70 px-2.5 py-1 text-[11px] font-bold text-white
          opacity-0 transition hover:bg-black focus:opacity-100 group-hover:opacity-100`,style:{opacity:.55},children:`새로고침`})]})}function ye(e,t){if(e.kind===`text`)return t||`(그냥 넘어감)`;let n=(Array.isArray(t)?t:[t]).map(t=>e.options.find(e=>e.value===t)?.label).filter(Boolean);return n.length?n.join(`, `):`(고르지 않음)`}function be(){let[e,t]=(0,l.useState)([]),[n,r]=(0,l.useState)(null),[i,a]=(0,l.useState)(!0),[o,s]=(0,l.useState)(null),[c,u]=(0,l.useState)(null),d=(0,l.useRef)(null),f=e=>Object.fromEntries(e.map(e=>[e.q.id,e.value])),p=(0,l.useCallback)(async e=>{a(!0),u(null);try{let t=await fe.ask(f(e));r(t)}catch(e){u(e.message||`질문을 가져오지 못했습니다`)}finally{a(!1)}},[]);(0,l.useEffect)(()=>{p([])},[p]),(0,l.useEffect)(()=>{d.current?.scrollIntoView({behavior:`smooth`,block:`end`})},[e.length,n,i]);let m=i=>{let a=[...e,{q:n,value:i}];t(a),r(null),p(a)},h=n=>{let i=e.slice(0,n);t(i),s(null),r(null),p(i)},g=async()=>{a(!0),u(null);try{s(await fe.build(f(e)))}catch(e){u(e.message||`만들지 못했습니다`)}finally{a(!1)}},_=()=>{t([]),s(null),r(null),p([])},v=!i&&!n;return(0,D.jsxs)(`div`,{className:`flex h-full flex-col lg:flex-row`,children:[(0,D.jsxs)(`div`,{className:`flex min-h-0 flex-col ${o?`lg:w-[46%] lg:border-r lg:border-white/10`:`mx-auto w-full max-w-3xl`}`,children:[(0,D.jsxs)(`header`,{className:`flex items-center gap-3 px-5 py-4`,children:[(0,D.jsx)(`div`,{className:`flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 text-lg`,children:`🤖`}),(0,D.jsxs)(`div`,{className:`min-w-0`,children:[(0,D.jsx)(`h1`,{className:`text-[15px] font-black text-white`,children:`대화형 코딩 AI`}),(0,D.jsx)(`p`,{className:`text-[11px] text-zinc-500`,children:`몇 가지만 물어보고 바로 만들어 드립니다`})]}),e.length>0&&(0,D.jsx)(`button`,{onClick:_,className:`ml-auto rounded-lg border border-white/10 px-3 py-1.5 text-[11px] font-bold text-zinc-400 transition hover:bg-white/10`,children:`처음부터`})]}),(0,D.jsxs)(`div`,{className:`min-h-0 flex-1 overflow-y-auto px-5 pb-4`,children:[e.map((e,t)=>(0,D.jsxs)(`div`,{className:`rise mb-5`,children:[(0,D.jsx)(xe,{children:e.q.text}),(0,D.jsx)(`div`,{className:`mt-2 flex justify-end`,children:(0,D.jsx)(`button`,{onClick:()=>h(t),title:`눌러서 다시 고르기`,className:`max-w-[85%] rounded-2xl rounded-br-md border border-violet-400/30 bg-violet-500/15 px-4 py-2
                    text-left text-[13.5px] font-bold text-violet-100 transition hover:border-violet-300/60 hover:bg-violet-500/25`,children:ye(e.q,e.value)})})]},t)),n&&(0,D.jsxs)(`div`,{className:`rise mb-4`,children:[(0,D.jsx)(xe,{children:n.text}),n.hint&&(0,D.jsx)(`p`,{className:`mb-3 ml-11 mt-1.5 text-[12px] leading-relaxed text-zinc-500`,children:n.hint}),(0,D.jsx)(`div`,{className:`ml-11 mt-3`,children:(0,D.jsx)(O,{question:n,onAnswer:m})})]}),i&&(0,D.jsx)(`div`,{className:`mb-4 flex items-center gap-1.5 pl-11 text-zinc-600`,children:[0,1,2].map(e=>(0,D.jsx)(`span`,{className:`dot h-1.5 w-1.5 rounded-full bg-current`,style:{animationDelay:`${e*.18}s`}},e))}),c&&(0,D.jsxs)(`div`,{className:`rise mb-4 rounded-2xl border border-rose-400/30 bg-rose-500/10 px-4 py-3`,children:[(0,D.jsx)(`p`,{className:`text-[13px] font-bold text-rose-200`,children:c}),(0,D.jsx)(`button`,{onClick:()=>p(e),className:`mt-2 text-[12px] font-bold text-rose-300 underline`,children:`다시 시도`})]}),v&&!o&&!c&&(0,D.jsxs)(`div`,{className:`rise mb-4`,children:[(0,D.jsx)(xe,{children:`다 물어봤어요. 이제 만들어 드릴게요!`}),(0,D.jsx)(`div`,{className:`ml-11 mt-3`,children:(0,D.jsx)(`button`,{onClick:g,className:`w-full rounded-2xl bg-gradient-to-r from-violet-500 to-fuchsia-500 py-3.5
                    text-[15px] font-black text-white transition hover:brightness-110`,children:`✨ 만들기`})})]}),(0,D.jsx)(`div`,{ref:d})]})]}),o&&(0,D.jsx)(`div`,{className:`min-h-0 flex-1 border-t border-white/10 lg:border-t-0`,children:(0,D.jsx)(_e,{result:o,onRestart:_})})]})}function xe({children:e}){return(0,D.jsxs)(`div`,{className:`flex gap-3`,children:[(0,D.jsx)(`div`,{className:`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/[.06] text-[15px]`,children:`🤖`}),(0,D.jsx)(`div`,{className:`max-w-[85%] rounded-2xl rounded-tl-md border border-white/10 bg-white/[.04] px-4 py-2.5
        text-[14px] leading-relaxed text-zinc-100`,children:e})]})}(0,u.createRoot)(document.getElementById(`root`)).render((0,D.jsx)(l.StrictMode,{children:(0,D.jsx)(be,{})}));