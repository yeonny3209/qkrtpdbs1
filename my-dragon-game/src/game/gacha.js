/* ==================================================================
   ...
   [한정 소환]  일반 75% · 레어 15% · 에픽 4%   · 레전드 6%
   [천장]       한정 소환 80회까지 레전드가 없으면 한정 레전드 확정
   [50:50]      레전드가 떴을 때 절반 확률로 상시 레전드가 대신 나온다
   [연속 방지]  상시 레전드가 나온 다음 레전드는 무조건 한정 레전드
   [초기화]     레전드(한정·상시 무관)를 얻으면 천장 스택이 0으로

   ------------------------------------------------------------------
   ⚠ 사양 충돌 메모
   기획서에는 한정 소환의 레전드가 "한정 0.5% / 상시 5.5%"로 적혀 있는데,
   같은 문단의 핵심 메커니즘은 "레전드 획득 시 50% 확률로 상시"라고 되어 있다.
   0.5 : 5.5 는 50:50 이 아니라 8:92 라서 둘이 동시에 성립할 수 없다.

   여기서는 "핵심 메커니즘"이라고 못박힌 50:50 을 정답으로 보고,
   레전드 총량 6%(=0.5+5.5)는 기획서 숫자를 그대로 지켰다.
   숫자대로 8:92 로 돌리고 싶으면 LIMITED_WIN_RATE 만 0.5 → 0.0833 으로
   바꾸면 된다. 다른 코드는 손댈 필요가 없다.
   ================================================================== */
...
export const LIMITED_WIN_RATE = 0.5     // 레전드 중 한정이 나올 확률 (위 메모 참고)
...
export const BANNERS = {
  ...
  limited: {
    ...
    rates: { common: 7500, rare: 1500, epic: 400, legend: 600 },   // 75 / 15 / 4 / 6 %
    pity: PITY,
  },
}
const TOTAL = 10000
